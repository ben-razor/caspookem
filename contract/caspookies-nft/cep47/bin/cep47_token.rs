...
#[repr(u16)]
enum FailureCode {
    AllMinted= 0,  // 65,536 as an ApiError::User
}
...
fn mint() {
    ...
    let next_token_id = NFTToken::default().total_supply();

    if next_token_id.as_u32() > 79 {
        runtime::revert(ApiError::from(FailureCode::AllMinted));
    }

    let token_ids = Vec::from([next_token_id]);
    let token_uri = format!("ipfs://Qmdb9FCJYCRihaQFJWhPgV5ZSQPLNbvmx7uniAfrh89cGt/{}.json", next_token_id.as_u32());

    let mut metadata = Meta::new();
    metadata.insert(String::from("token_uri"), token_uri);
    let mut token_metas = Vec::<Meta>::new();
    token_metas.push(metadata);
